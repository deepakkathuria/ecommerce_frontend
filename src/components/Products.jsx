...
const Products = () => {
  ...
  useEffect(() => {
    const getProducts = async () => {
      setLoading(true);
      try {
        const url = new URL(apiUrl("/products"), typeof window !== "undefined" ? window.location.origin : "http://127.0.0.1:3000");
    ...
    try {
      if (alreadyInWishlist) {
        const response = await fetch(
          apiUrl(`/wishlist/remove/${product.id}`),
          {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
          }
