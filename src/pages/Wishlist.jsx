...
const Wishlist = () => {
  ...
  const handleRemoveFromWishlist = async (productId) => {
    ...
    try {
      const response = await fetch(apiUrl(`/wishlist/remove/${productId}`), {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
              ...
              <div key={productId} className="col-6 col-md-4 col-lg-3 mb-4">
                <div className="product-card-zara position-relative">
                  <div className="product-image-container">
                    <Link href={`/product/${generateProductSlug(item.name, productId)}`}>
                      <img
                  ...
                  <div className="product-info">
                    <Link href={`/product/${generateProductSlug(item.name, productId)}`} className="product-title">
                      {item.name}
                    </Link>
